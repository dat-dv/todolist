...
const TodoItem: React.FC<TTodoItemProps> = ({
  ...
  if (!todo?.id) return <></>;
  return (
    <li className={`flex items-start gap-4 ${className}`} {...rest}>
      <button
        disabled={disabled}
        onClick={() => onRemove(todo?.id)}
        aria-label="Delete"
        className="flex items-center justify-center shrink-0 h-9 w-9 rounded-md bg-red-500 hover:bg-red-600 text-white disabled:opacity-50"
      >
        <svg
      ...
      <div
        className={`flex-1 font-mono text-[20px] ${
      ...
      <div className="flex items-center space-x-2">
        {isEditing ? (
          <button
            ...
            className="flex items-center justify-center shrink-0 h-9 w-9 rounded-md bg-gray-500 hover:bg-gray-600 text-white"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-4 w-4"
          ...
          <button
            ...
            aria-label="Edit"
            className="flex items-center justify-center shrink-0 h-9 w-9 rounded-md bg-blue-500 hover:bg-blue-600 text-white disabled:opacity-50"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-4 w-4"
